<script lang="ts">
    ...
    import { ChoroplethChart, topojson, ChoroplethController, GeoFeature, ColorScale, ProjectionScale } from 'chartjs-chart-geo';
...
//    let mapData = countries.map((d) => ({feature: d, value: Math.floor(Math.random() * 1000)}));
     ...
     let mapData = countries.map((d) => ({
            feature: d, value: analyticsData.has(d.properties.name)? analyticsData.get(d.properties.name) : 0
     }))
